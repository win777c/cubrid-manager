...
remove_files() {
    rm -f ./${PRODUCT_NAME}_Setup_with_jre.sh ${PRODUCT_NAME}-product.tar.gz ${PRODUCT_NAME}-product.tar ./install.${PRODUCT_NAME}.tmp ./version.sh COPYING
}
atexit() {
    remove_files
    while [ $? != 0 ];
...
./${PRODUCT_NAME}_Setup_with_jre.sh "$0"
