...
public class TestTaskTest extends
		TestCase {
	...
	public void testUpdateMessage() {
		...
		for (Class c : list) {
			...
			sb.append("public void testModel").append(c.getSimpleName()).append(
					...
					" bean= new ").append(c.getSimpleName()).append("();").append(
					"\n");
			...
			for (Field f : fields) {
				...
				for (Method m : methods) {
					if (m.getName().equalsIgnoreCase("set" + fName)) {
						...
						if (!map.containsKey(m.getName().replaceFirst("set",
								"get")))
							map.put(m.getName().replaceFirst("set", "get"),
									m.getName().replaceFirst("set", "get"));
					} else if (m.getName().equalsIgnoreCase("add" + fName)) {
						sb.append(" bean.").append(m.getName()).append("(").append(
								"new ").append(
						...
						if (!map.containsKey(m.getName()))
							map.put(m.getName(), m.getName());
						if (!map.containsKey(m.getName().replaceFirst("add",
								"get")))
							map.put(m.getName().replaceFirst("add", "get"),
									m.getName().replaceFirst("add", "get"));
					}
				}
